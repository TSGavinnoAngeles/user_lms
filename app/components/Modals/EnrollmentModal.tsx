...
const EnrollmentModal: React.FC<EnrollmentModalProps> = ({
  ...
  const onSubmit = (name: string) => {
    startTransition(() => {
      toast.promise(
        enrollStudent({ courseName: name }).then((data) => {
          if (data && data.error) {
    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="bg-slate-900/20 backdrop-blur p-8 fixed inset-0 z-50 grid place-items-center overflow-auto cursor-pointer  "
        >
          <motion.div
